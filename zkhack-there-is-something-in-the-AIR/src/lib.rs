...
impl AccessSet {
    ...
    pub fn make_signal(&self, priv_key: &PrivKey, topic: &str) -> Signal {
        ...
        let nullifier = priv_key.get_incorrect_nullifier(topic);
        ...
        let trace = prover.build_incorrect_trace(priv_key, key_idx, topic, &key_path);
...
impl PrivKey {
    ...
    pub fn get_incorrect_nullifier(&self, topic: Digest) -> Digest {
        let key: Digest = self.0.into();
        let values = [key, topic];

        let mut state = [Felt::ZERO; 12];
        state[4..12].copy_from_slice(Digest::digests_as_elements(&values));
        state[0] = Felt::new(10 as u64);

        // apply the Rescue permutation and return the first four elements of the state
        Rescue::apply_permutation(&mut state);

        Digest::new(state[4..8].try_into().unwrap())
    }
