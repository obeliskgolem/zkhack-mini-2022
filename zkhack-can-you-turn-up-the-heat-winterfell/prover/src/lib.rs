...
pub trait Prover {
        ...
        H: ElementHasher<BaseField = Self::BaseField>,
    {
        ...
        let mut fake_evaluations_low_degree: Vec<E> = vec![E::ZERO; 16];

        for i in 0..16 {
                fake_evaluations_low_degree[i] = deep_evaluations[8*i];
        }

        let twiddles = math::fft::get_twiddles::<E::BaseField>(fake_evaluations_low_degree.len());
        let inv_twiddles = math::fft::get_inv_twiddles(fake_evaluations_low_degree.len());

        math::fft::interpolate_poly_with_offset(&mut fake_evaluations_low_degree, &inv_twiddles, air.domain_offset());
        let mut fake_evaluations: Vec<E> = Vec::new();
        for i in 0..deep_evaluations.len() {
            let eval = math::polynom::eval(&fake_evaluations_low_degree[..], deep_composer.x_coordinates[i].into());
            fake_evaluations.push(eval);
        }

        //deep_evaluations.iter().zip(fake_evaluations.iter()).for_each(|a, b| { if a == b {println!("1");} else {println!("0")}});
        let count = deep_evaluations.iter().zip(fake_evaluations.iter()).filter(|(a, b)| a == b).count();
